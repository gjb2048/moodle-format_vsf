...
/**
 * Course category level icon modification.
 *
 ...
 * - store each (btw completely optional) image in mdl_files under the coursecategory
...
$coursecat = \core_course_category::get($id);

$params = ['id' => $id];
$url = new moodle_url('/course/format/vsf/coursecaticons.php', $params);
...
$form = new format_vsf\local\modicon\caticonsform($url, ['coursecat' => $coursecat, 'context' => $context]);
if ($form->is_cancelled()) {
    redirect($PAGE->url);
} else if ($form->save_data()) {
    \core\notification::success(get_string('modicons:coursecat:changes:saved', 'format_vsf'));
    redirect($PAGE->url);
}

$oneupurl = new moodle_url($CFG->wwwroot . '/course/format/vsf/modicons.php');

echo $renderer->header();
echo $renderer->heading(get_string('modicon:image:coursecat:head', 'format_vsf',
        $coursecat->name));
echo html_writer::div(get_string('modicon:image:coursecat:desc', 'format_vsf',
