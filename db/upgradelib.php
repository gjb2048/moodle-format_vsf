...
/**
 * This method finds all courses in 'vsf' format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@link format_vsf_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 */
function format_vsf_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'vsf', 'format2' => 'vsf', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_vsf_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'vsf', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_vsf_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
